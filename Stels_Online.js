    ...
    function uaflix(component, _object) {
      ...
      function tryPlayerUrls(players, pageReferer, success, fail) {
        ...
        function nextPlayer(lastError) {
          ...
          var ri = 0;
          function tryReferer(lastRefererError) {
            if (ri >= refs.length) {
              nextPlayer(lastRefererError || 'no file');
              return;
            }
            var rr = refs[ri++];
            var requestCandidates = uaflixZetvideoRequestCandidates(player.iframe);
            var ui = 0;
            function tryRequestUrl(lastRequestError) {
              if (ui >= requestCandidates.length) {
                tryReferer(lastRequestError || 'no file');
                return;
              }
              var req = requestCandidates[ui++];
              ...
              requestText(req.url, function (playerHtml) {
                ...
                if (data.file) {
                  stelsLog('uaflix-player-success', { player: player.iframe, request_url: req.url, request_mode: req.mode, proxied: req.proxied, referer: rr, file: data.file });
                  success(data, player);
                } else {
                  stelsLog('uaflix-player-no-file', {
                    ...
                    preview: uaflixCompact(playerHtml || '', 250)
                  });
