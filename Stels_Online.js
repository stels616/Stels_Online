(function() {
  ...
  // Stels_Online: Lampac/LampaUaNg-based engine build.
  // This version uses lampaua.mooo.com endpoints for sources and streams instead of the BanderaOnline bbe API.
  ...
  var STELS_VERSION = '3.1.0-stels-lampac-engine-debug';
  var STELS_DEBUG_KEY = 'stels_online_lampac_debug_events_v3';
  var STELS_TOKEN_KEY = 'stels_online_lampac_token';
  ...
  function sanitizeUrlForLog(url) {
    var text = normalizeText(url);
    if (!text) return '';
    return text
      .replace(/([?&](?:token|account_email|uid|nws_id|kit_aesgcmkey|hdrezka_auth_id|filmix_token|filmix_device_id)=)[^&]*/ig, '$1***')
      .replace(/(X-Kit-AesGcm[:=])[^&\s]+/ig, '$1***');
  }
  ...
  function summarizeValue(value, depth) {
    ...
    if (typeof value == 'string') return truncateText(sanitizeUrlForLog(value), 900);
    ...
    if (Array.isArray(value)) {
      return {
        type: 'array',
        count: value.length,
        sample: value.slice(0, 8).map(function (item) { return summarizeValue(item, depth + 1); })
      };
    }
    if (typeof value == 'object') {
      var keys = Object.keys(value);
      var out = { type: 'object', count: keys.length };
      keys.slice(0, 24).forEach(function (key) {
        out[key] = summarizeValue(value[key], depth + 1);
      });
      if (keys.length > 24) out.__truncated_keys = keys.length - 24;
      return out;
    }
    return String(value);
  }
  ...
  function getDebugEvents() {
    var events = Lampa.Storage.get(STELS_DEBUG_KEY, []);
    return Array.isArray(events) ? events : [];
  }

  function setDebugEvents(events) {
    Lampa.Storage.set(STELS_DEBUG_KEY, events.slice(-250));
  }

  function debugLog(event, data) {
    try {
      var events = getDebugEvents();
      events.push({
        time: nowIso(),
        event: event,
      ...
      if (window.console && console.log) console.log('[Stels_Online]', event, data || {});
    } catch (e) {}
  }

  function storageSummary(key) {
    var value = Lampa.Storage.get(key, null);
    if (Array.isArray(value)) {
      return { type: 'array', count: value.length, sample: value.slice(0, 20) };
    }
    if (value && typeof value == 'object') {
      return { type: 'object', count: Object.keys(value).length, keys: Object.keys(value).slice(0, 40) };
    }
    return value;
  }
  ...
  function buildDebugDump() {
    return {
      plugin: 'Stels_Online',
      version: STELS_VERSION,
      engine: 'Lampac/LampaUaNg',
      time: nowIso(),
      host: Defined.localhost,
      token_set: !!getLampacToken(),
      storage: {
        stels_online_lampac_token_set: !!getLampacToken(),
        online_balanser: storageSummary('online_balanser'),
        online_last_balanser: storageSummary('online_last_balanser'),
        active_balanser: storageSummary('active_balanser'),
        lampac_unic_id: storageSummary('lampac_unic_id'),
        lampac_nws_id: storageSummary('lampac_nws_id'),
        old_BO_SOURCES_count: (function(){ var v=Lampa.Storage.get('BO_SOURCES', []); return Array.isArray(v) ? v.length : 0; })(),
        old_stels_online_balanser: storageSummary('stels_online_balanser')
      },
  ...
  function stringifyDebugDump() {
    try { return JSON.stringify(buildDebugDump(), null, 2); }
    catch (e) { return 'Stels_Online debug stringify error: ' + (e && e.message ? e.message : e); }
  }

  function openDebugLogModal() {
    var text = 'Stels_Online debug log\n' + stringifyDebugDump();
    var controller = Lampa.Controller.enabled().name;
    var html = $('<div class="about"><div style="margin-bottom:1em">Версія: <b>' + STELS_VERSION + '</b><br>Движок: Lampac/LampaUaNg<br>Token: ' + (getLampacToken() ? 'задано' : 'не задано') + '</div><textarea readonly style="width:100%;height:22em;box-sizing:border-box;background:rgba(0,0,0,.25);color:currentColor;border:1px solid rgba(255,255,255,.25);border-radius:.4em;padding:.8em;font-size:.8em;white-space:pre-wrap"></textarea><div class="simple-button selector" data-action="copy" style="margin-top:1em">Скопіювати лог</div><div class="simple-button selector" data-action="clear" style="margin-top:.6em">Очистити лог</div></div>');
    html.find('textarea').val(text);
    html.on('hover:enter click', '[data-action="copy"]', function () {
      if (Lampa.Utils && Lampa.Utils.copyTextToClipboard) {
        Lampa.Utils.copyTextToClipboard(text, function () { Lampa.Noty.show('Лог скопійовано'); }, function () { Lampa.Noty.show('Не вдалося скопіювати лог'); });
      } else if (window.prompt) {
        window.prompt('Скопіюй лог', text);
      }
    });
    html.on('hover:enter click', '[data-action="clear"]', function () {
      setDebugEvents([]);
      Lampa.Noty.show('Лог очищено');
      html.find('textarea').val('Stels_Online debug log\n' + stringifyDebugDump());
    });
    Lampa.Modal.open({
  ...
  function getLampacToken() {
    return normalizeText(Lampa.Storage.get(STELS_TOKEN_KEY, ''));
  }

  function setLampacToken(value) {
    Lampa.Storage.set(STELS_TOKEN_KEY, normalizeText(value));
    debugLog('token-updated', { token_set: !!getLampacToken() });
    try { Lampa.Settings.update(); } catch (e) {}
  }

  function openLampacTokenInput() {
    var current = getLampacToken();
    if (Lampa.Input && typeof Lampa.Input.edit == 'function') {
      Lampa.Input.edit({
        ...
        setLampacToken(value);
        Lampa.Noty.show(value ? 'Token збережено' : 'Token очищено');
      });
      return;
    }
    if (window.prompt) {
      var next = window.prompt('Lampac token', current);
      if (next !== null) setLampacToken(next);
    }
  }

  function clearLegacyStelsStorage() {
    ['BO_SOURCES','BO_SOURCES_SORT','BO_SOURCES_HIDE','stels_online_balanser','stels_online_last_balanser','bandera_online_balanser','bandera_online_last_balanser'].forEach(function (key) {
      try { Lampa.Storage.remove(key); } catch (e) { try { Lampa.Storage.set(key, null); } catch (_) {} }
    });
    debugLog('legacy-storage-cleared', {});
    Lampa.Noty.show('Старий кеш очищено. Перезапусти Lampa.');
  }
  ...
  function initSettings() {
    ...
    try {
      SettingsApi.addComponent({
        ...
        name: 'Stels_Online',
        icon: '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M3 4h3v9.2c0 2.2 1 3.6 2.8 3.6s2.8-1.4 2.8-3.6V4h3v9.2c0 4-2.3 6.9-5.8 6.9S3 17.2 3 13.2V4z" fill="#FFD700"/><path d="M17.4 4l5.2 16h-3.1l-.9-3h-4.9l-1 3H9.7L15 4h2.4zm.4 10.4l-1.6-5.3-1.7 5.3h3.3z" fill="#0057B7"/></svg>'
      });
      SettingsApi.addParam({
        component: 'stels_online',
        param: { name: 'stels_online_version', type: 'button' },
        field: { name: 'Версія: ' + STELS_VERSION },
        onChange: function () { openDebugLogModal(); }
      });
      SettingsApi.addParam({
        component: 'stels_online',
        param: { name: 'stels_online_lampac_token_button', type: 'button' },
        field: { name: 'Lampac token: ' + (getLampacToken() ? 'задано' : 'не задано') },
        onChange: function () { openLampacTokenInput(); }
      });
      SettingsApi.addParam({
        component: 'stels_online',
        param: { name: 'stels_online_debug_export', type: 'button' },
        field: { name: 'Відкрити / скопіювати лог' },
        onChange: function () { openDebugLogModal(); }
      });
      SettingsApi.addParam({
        component: 'stels_online',
        param: { name: 'stels_online_clear_legacy', type: 'button' },
        field: { name: 'Очистити старий кеш Bandera/Stels' },
        onChange: function () { clearLegacyStelsStorage(); }
      });
    } catch (e) {
      debugLog('settings-init-error', { message: e && e.message ? e.message : e });
    }
  }
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      debugLog('withsearch-start', { url: 'http://lampaua.mooo.com/lite/withsearch' });
      network.silent(account('http://lampaua.mooo.com/lite/withsearch'), function(json) {
        balansers_with_search = json;
        debugLog('withsearch-success', { count: Array.isArray(json) ? json.length : 0, sample: Array.isArray(json) ? json.slice(0, 20) : json });
      }, function(e) {
        balansers_with_search = [];
        debugLog('withsearch-error', { error: e && (e.status || e.message || e) });
      });
    }
    ...
    this.externalids = function() {
      return new Promise(function(resolve, reject) {
        if (!object.movie.imdb_id || !object.movie.kinopoisk_id) {
          ...
          debugLog('externalids-start', { url: sanitizeUrlForLog(url), movie_id: object.movie.id, title: object.movie.title || object.movie.name });
          network.timeout(10000);
          network.silent(account(url), function(json) {
            debugLog('externalids-success', { keys: json ? Object.keys(json) : [], json: json });
            for (var name in json) {
              object.movie[name] = json[name];
            }
            resolve();
          }, function(e) {
            debugLog('externalids-error', { error: e && (e.status || e.message || e) });
            resolve();
          }, false, {
              headers: addHeaders()
		  });
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        debugLog('start-source-input', { isArray: Array.isArray(json), count: Array.isArray(json) ? json.length : 0, sample: Array.isArray(json) ? json.slice(0, 12) : json });
        rememberAccessGroup(json);
        json.forEach(function(j) {
          setSource(j);
        });
        rebuildFilterSources();
        debugLog('start-source-built', { source_keys: filter_sources, sources: sources });
        if (filter_sources.length) {
          ...
          debugLog('start-source-selected', { balanser: balanser, title: sourceTitle(balanser), url: sanitizeUrlForLog(source), custom_select: object.lampac_custom_select });
          resolve(json);
        } else {
          debugLog('start-source-empty', { input: json });
          reject();
        }
      });
    };
    this.lifeSource = function() {
      ...
      return new Promise(function(resolve, reject) {
        ...
        debugLog('life-source-start', { url: sanitizeUrlForLog(url), memkey: _this3.memkey || '' });
        var red = false;
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            life_wait_times++;
            debugLog('life-source-response', { wait: life_wait_times, ready: json && json.ready, online_count: json && json.online && json.online.length, sample: json && json.online ? json.online.slice(0, 10) : json });
            filter_sources = [];
            sources = {};
            rememberAccessGroup(json);
    ...
    this.createSource = function() {
      ...
      return new Promise(function(resolve, reject) {
        ...
        debugLog('create-source-start', { url: sanitizeUrlForLog(url), movie: { id: object.movie && object.movie.id, title: object.movie && (object.movie.title || object.movie.name), imdb_id: object.movie && object.movie.imdb_id, kinopoisk_id: object.movie && object.movie.kinopoisk_id, tmdb_id: object.movie && (object.movie.tmdb_id || object.movie.id), serial: object.movie && !!object.movie.name } });
        network.timeout(15000);
        network.silent(account(url), function(json) {
          debugLog('create-source-response', { accsdb: json && json.accsdb, life: json && json.life, ready: json && json.ready, memkey: json && json.memkey, isArray: Array.isArray(json), count: Array.isArray(json) ? json.length : (json && json.online ? json.online.length : 0), sample: Array.isArray(json) ? json.slice(0, 10) : (json && json.online ? json.online.slice(0, 10) : json) });
          if (json.accsdb) return reject(json);
          rememberAccessGroup(json);
          if (json.life) {
          ...
          debugLog('create-source-error', { error: e && (e.status || e.message || e), url: sanitizeUrlForLog(url) });
          reject(e);
        }, false, {
            headers: addHeaders()
		  });
    ...
    this.find = function() {
      debugLog('find-start', { balanser: balanser, source_title: sources[balanser] && sources[balanser].name, url: sanitizeUrlForLog(source), locked: sources[balanser] && sources[balanser].locked, show: sources[balanser] && sources[balanser].show });
    ...
    this.request = function(url) {
      var _this_request = this;
      number_of_requests++;
      if (number_of_requests < 10) {
        var finalUrl = account(url);
        debugLog('request-start', { count: number_of_requests, balanser: balanser, url: sanitizeUrlForLog(finalUrl) });
        network["native"](finalUrl, function(response) {
          var text = typeof response == 'string' ? response : (response ? JSON.stringify(response) : '');
          debugLog('request-success', {
            balanser: balanser,
            url: sanitizeUrlForLog(finalUrl),
            response_type: typeof response,
            length: text.length,
            has_videos_item: text.indexOf('videos__item') >= 0,
            has_videos_button: text.indexOf('videos__button') >= 0,
            has_rch: text.indexOf('"rch"') >= 0 || text.indexOf('rch') >= 0,
            has_accsdb: text.indexOf('accsdb') >= 0,
            snippet: truncateText(text, 900)
          });
          _this_request.parse(response);
        }, function(e) {
          debugLog('request-error', { balanser: balanser, url: sanitizeUrlForLog(finalUrl), error: e && (e.status || e.message || e) });
          _this_request.doesNotAnswer(e);
        }, false, {
          dataType: 'text',
          headers: addHeaders()
        });
        ...
        debugLog('request-limit-empty', { balanser: balanser, url: sanitizeUrlForLog(url), count: number_of_requests });
        this.empty();
      }
    };
    ...
    this.getFileUrl = function(file, call, waiting_rch) {
      debugLog('get-file-url-start', { balanser: balanser, waiting_rch: !!waiting_rch, file: file });
	  var _this = this;
      ...
      if(Lampa.Storage.field('player') !== 'inner' && file.stream && Lampa.Platform.is('apple')){
        ...
        var fileUrl = account(file.url);
        debugLog('get-file-url-request', { balanser: balanser, url: sanitizeUrlForLog(fileUrl), method: file && file.method });
        network["native"](fileUrl, function(json) {
          debugLog('get-file-url-response', { balanser: balanser, response_type: typeof json, has_url: !!(json && json.url), has_rch: !!(json && json.rch), json: json });
			if(json.rch){
				if(waiting_rch) {
					waiting_rch = false;
					Lampa.Loading.stop();
          ...
          debugLog('get-file-url-error', { balanser: balanser, url: sanitizeUrlForLog(fileUrl), error: e && (e.status || e.message || e) });
          Lampa.Loading.stop();
          call(false, {});
        }, false, {
            headers: addHeaders()
    ...
    this.display = function(videos) {
      ...
      this.draw(videos, {
        onEnter: function onEnter(item, html) {
          debugLog('play-enter', { balanser: balanser, item: item });
    ...
    this.parse = function(str) {
      debugLog('parse-start', { balanser: balanser, input_type: typeof str, input_length: (typeof str == 'string' ? str.length : 0), input_snippet: typeof str == 'string' ? truncateText(str, 700) : str });
      ...
      try {
        ...
        debugLog('parse-items', { balanser: balanser, items_count: items.length, buttons_count: buttons.length, items_sample: items.slice(0, 8), buttons_sample: buttons.slice(0, 8) });
        if (items.length == 1 && items[0].method == 'link' && !items[0].similar) {
          ...
          debugLog('parse-classified', { videos_count: videos.length, similar_count: similar.length, link_count: items.filter(function(v){ return v.method == 'link'; }).length });
          if (videos.length) {
            if (buttons.length) {
        ...
        debugLog('parse-error', { message: e && e.message ? e.message : e, stack: e && e.stack ? e.stack : '' });
     ...
     */
    this.empty = function() {
      debugLog('empty', { balanser: balanser, source: sources[balanser] });
    ...
    this.noConnectToServer = function(er) {
      debugLog('no-connect-to-server', { balanser: balanser, error: er });
    ...
    this.doesNotAnswer = function(er) {
      debugLog('does-not-answer', { balanser: balanser, error: er, source: sources[balanser] });
      var _this9 = this;
      this.reset();
  ...
  function startPlugin() {
    window.Stels_Online_plugin = true;
    debugLog('plugin-start', { version: STELS_VERSION, host: Defined.localhost, location: location.href, token_set: !!getLampacToken() });
    var manifst = {
      type: 'video',
      version: STELS_VERSION,
      name: 'Stels_Online',
    ...
    initSettings();
	addSourceSearch('Stels_Online', 'spider');
	addSourceSearch('Stels_Online - Anime', 'spider/anime');
...
.selectbox-item__icon svg[viewBox="0 0 392.697 392.697"] path {\
  opacity: 0 !important;\
   ...
   Lampa.Component.add('Stels_Online', component); //то же самое
    ...
    function addButton(e) {
      if (e.render.find('.stels-online--button').length) return;
  ...
  if (!window.Stels_Online_plugin) startPlugin();
